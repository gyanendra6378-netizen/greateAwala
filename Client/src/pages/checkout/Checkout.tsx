...
const Checkout = () => {
  const [shippingMethod, setShippingMethod] =
    useState<"standard" | "express" | "sameDay">("standard");
  const [paymentMethod, setPaymentMethod] = useState("UPI");

  const checkoutSteps = [
    { label: "checkout method", icon: <Pencil size={20} /> },
    { label: "billing information", icon: <User size={20} /> },
  ...
  const renderStepComponent = (label: (typeof checkoutSteps)[number]["label"]) => {
    switch (label) {
